...
- Folder guides:
  ...
  - The dev build path sets `DOCKER_LOCAL=1` during `docker:build:dev`. Webpack will rewrite a default `http://localhost:<port>` API base to `http://host.docker.internal:<port>` so the container can reach your host services.
