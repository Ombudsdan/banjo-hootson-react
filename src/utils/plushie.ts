...
export function formatPossessivePlushieName(name: string): string {
  if (!name || !name.trim()) throw new Error('Cannot format possessive name for empty string');
...
function getPlushieNameFromEventName(text: string): string {
  ...
  const plushieName = apostropheIndex === -1 ? text : text.substring(0, apostropheIndex);
...
export function getPropertiesFromPlushieBirthdayEventName(event: IPlushieBirthday): IPlushieBirthdayDisplayProperties {
